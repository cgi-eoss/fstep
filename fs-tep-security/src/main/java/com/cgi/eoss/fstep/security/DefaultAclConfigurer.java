...
public class DefaultAclConfigurer {
    ...
    public void initAcls() throws Exception {
        ...
        ensureAcl(Project.class, defaultProject, FstepPermission.PUBLIC, FstepPermission.READ);
    }
     ...
     */
    private void ensureAcl(Class<? extends FstepEntity> entityClass, FstepEntity entity, GrantedAuthority authority, FstepPermission permission) {
        PrincipalSid ownerSid = FstepEntityWithOwner.class.isAssignableFrom(entityClass)
                ? new PrincipalSid(User.DEFAULT.getName())
                : new PrincipalSid(((FstepEntityWithOwner) entity).getOwner().getName());
