...
public class ServiceFilesApiIT {
    ...
    public void testSave() throws Exception {
        ...
        mockMvc.perform(post("/api/serviceFiles").header("REMOTE_USER", fstepExpertUser.getName()).content(newFile1))
                .andExpect(status().isCreated())
                .andExpect(header().string("Location", matchesPattern(".*/serviceFiles/\\d+$")));
        mockMvc.perform(post("/api/serviceFiles").header("REMOTE_USER", fstepExpertUser.getName()).content(newFile2))
                .andExpect(status().isCreated())
                .andExpect(header().string("Location", matchesPattern(".*/serviceFiles/\\d+$")));

        List<FstepServiceContextFile> serviceFiles = serviceFileDataService.findByService(svc);
    ...
    public void testFindByService() throws Exception {
        ...
        mockMvc.perform(get("/api/serviceFiles/search/findByService").param("service", svcUrl).header("REMOTE_USER", fstepUser.getName()))
                .andExpect(status().isForbidden());
        mockMvc.perform(get("/api/serviceFiles/search/findByService").param("service", svcUrl).header("REMOTE_USER", fstepContentAuthority.getName()))
    ...
    private String getServiceUrl() throws Exception {
        return JsonPath.compile("$._links.self.href").read(
                mockMvc.perform(get("/api/services/" + svc.getId()).header("REMOTE_USER", fstepContentAuthority.getName()))
