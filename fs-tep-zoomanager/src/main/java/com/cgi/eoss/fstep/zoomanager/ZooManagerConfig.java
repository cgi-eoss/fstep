...
public class ZooManagerConfig {
    ...
    @Bean
    public String javacClasspath(@Value("${fstep.zoomanager.javac.classpath}") String javacClasspath) {
        return javacClasspath;
    }

    @Bean
    public Path zcfgBasePath(@Value("${fstep.zoomanager.zcfg.path:/var/www/cgi-bin}") String zcfgBasePath) {
        return Paths.get(zcfgBasePath);
    }

    @Bean
    public Path zooServicesStubJar(@Value("${fstep.zoomanager.stub.jarFile:/var/www/cgi-bin/jars/fs-tep-services.jar}") String zooServicesStubJar) {
