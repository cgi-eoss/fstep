...
public class AppConfig {
	...
	@Bean
	public DockerClient dockerClient(@Value("${dockerUrl}") String dockerUrl) {
		DockerClientConfig dockerClientConfig = DefaultDockerClientConfig.createDefaultConfigBuilder()
				.withApiVersion(RemoteApiVersion.VERSION_1_19).withDockerHost(dockerUrl).build();

		return DockerClientBuilder.getInstance(dockerClientConfig).build();
	}

	@Value("${brokerUrl:vm://embeddedBroker}")
