...
define(['../fstepmodules', 'traversonHal'], function (fstepmodules, TraversonJsonHalAdapter) {
    ...
    fstepmodules.service('ProductService', ['$rootScope', 'CommunityService', 'UserService', 'MessageService', '$http', 'fstepProperties', '$q', 'traverson', '$timeout', function ( $rootScope, CommunityService, UserService, MessageService, $http, fstepProperties, $q, traverson, $timeout) {
        ...
        this.getService = function(service){
            var deferred = $q.defer();
            halAPI.from(rootUri + '/services/' + service.id + '?projection=detailedFstepService')
                       .newRequest()
                       .getResource()
                       .result
                       .then(
        ...
        this.createService = function(name, description, title){
            return $q(function(resolve, reject) {
                  var service = {
                          ...
                          dockerTag: 'fstep/' + name.toLowerCase(),
                          serviceDescriptor: {
                              description: description,
                              id: name,
                              title: title,
