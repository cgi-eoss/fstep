...
public class IptDownloader implements Downloader {
    ...
    @Autowired
    IptDownloader(OkHttpClient okHttpClient, FstepServerClient fstepServerClient) {
    ...
    public Path download(Path targetDir, URI uri) throws IOException {
        ...
        Credentials credentials = fstepServerClient.credentialsServiceBlockingStub().getCredentials(GetCredentialsParams.newBuilder().setHost(uri.getHost()).build());
