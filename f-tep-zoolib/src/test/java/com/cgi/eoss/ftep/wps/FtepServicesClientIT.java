...
public class FtepServicesClientIT {
    ...
    public void setUp() throws Exception {
        ...
        when(catalogueService.ingestOutputProduct(any(), any())).thenAnswer(invocation -> {
            OutputProductMetadata outputProductMetadata = (OutputProductMetadata) invocation.getArgument(0);
            Path outputPath = (Path) invocation.getArgument(1);
            FtepFile ftepFile = new FtepFile(URI.create("ftep://outputs/" + ingestedOutputsDir.relativize(outputPath)), UUID.randomUUID());
    ...
    public void launchApplication() throws Exception {
        ...
        assertThat(outputs.get("1"), containsInAnyOrder("ftep://outputs/" + launchedJobs.get(0).getExtId() + "/output_file_1"));
    ...
    public void launchProcessor() throws Exception {
        ...
        assertThat(outputs, is(notNullValue()));
        assertThat(outputs.get("output"), containsInAnyOrder("ftep://outputs/" + launchedJobs.get(0).getExtId() + "/output_file_1"));
    ...
    public void launchParallelProcessor() throws Exception {
        ...
        when(service.getDockerTag()).thenReturn("ftep/testservice1");
        when(service.getType()).thenReturn(FtepService.Type.PARALLEL_PROCESSOR);
        when(service.getServiceDescriptor()).thenReturn(serviceDescriptor);
        when(serviceDescriptor.getDataOutputs()).thenReturn(ImmutableList.of(
                FtepServiceDescriptor.Parameter.builder().id("output").build()
        ));
        ...
        Multimap<String, String> outputs = ftepServicesClient.launchService(userId, PARALLEL_SERVICE_NAME, jobId, inputs);
        ...
        assertThat(ImmutableSet.copyOf(outputs.get("output")), is(Seq.of(1, 2, 3).stream()
                .map(i -> {
                    ...
                    return "ftep://outputs/" + job.getExtId() + "/output_file_1";
                })
                .collect(toSet())));

        assertThat(Files.exists(workspace.resolve("Job_" + launchedJobs.get(0).getExtId()).resolve("FTEP-WPS-INPUT.properties")), is(false));
        assertThat(Files.readAllLines(workspace.resolve("Job_" + launchedJobs.get(1).getExtId()).resolve("FTEP-WPS-INPUT.properties")), is(ImmutableList.of(
                "sharedInputFoo=\"foo\"",
                "sharedInputBarBaz=\"bar,baz\"",
                "input=\"parallelInput1\""
        )));
        assertThat(Files.readAllLines(workspace.resolve("Job_" + launchedJobs.get(2).getExtId()).resolve("FTEP-WPS-INPUT.properties")), is(ImmutableList.of(
                "sharedInputFoo=\"foo\"",
                "sharedInputBarBaz=\"bar,baz\"",
                "input=\"parallelInput2\""
        )));
        assertThat(Files.readAllLines(workspace.resolve("Job_" + launchedJobs.get(3).getExtId()).resolve("FTEP-WPS-INPUT.properties")), is(ImmutableList.of(
                "sharedInputFoo=\"foo\"",
                "sharedInputBarBaz=\"bar,baz\"",
