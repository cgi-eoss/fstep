...
import com.cgi.eoss.fstep.search.fstep.opensearch.RestoSearchResult;
...
@Log4j2
public class FstepRestoSearchProvider implements SearchProvider {
    ...
    public FstepRestoSearchProvider(com.cgi.eoss.fstep.search.fstep.FstepSearchProperties fstepSearchProperties, OkHttpClient httpClient, ObjectMapper objectMapper, RestoService restoService) {
        this.baseUrl = fstepSearchProperties.getBaseUrl();
        this.client = httpClient.newBuilder()
                .addInterceptor(chain -> {
                    Request authenticatedRequest = chain.request().newBuilder()
                            .header("Authorization", Credentials.basic(fstepSearchProperties.getUsername(), fstepSearchProperties.getPassword()))
                            .build();
