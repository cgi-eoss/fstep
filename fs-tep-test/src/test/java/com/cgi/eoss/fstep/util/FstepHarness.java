...
public class FstepHarness {
    ...
    public FstepHarness() {
        environment = new DockerComposeContainer(Paths.get(System.getProperty("HARNESS_DIR"), "docker-compose.yml").toFile())
                .withExposedService("fstep-portal_1", 80);
    ...
    public void preScenario(Scenario scenario) {
        ...
        LOG.info("*** FS-TEP Services exposed on: {} ***", getFstepBaseUrl());
        ...
        getChromeWebDriver().get(getFstepBaseUrl() + "/app/");
    ...
    String getFstepBaseUrl() {
        return "http://" +
                hostIp + ":" +
                environment.getServicePort("fstep-portal_1", 80);
    }
    ...
    private void containersAreUp() {
        ImmutableMap.of(
            ...
            Request webRoot = new Request.Builder().url(getFstepBaseUrl() + path).build();
